...
export function CollaborationPortalProvider({ children }: { children: ReactNode }) {
  ...
  const getInitialPosition = () => {
    if (typeof window !== 'undefined') {
      return { x: Math.max(20, window.innerWidth - 420), y: Math.max(20, window.innerHeight - 620) };
    }
    return { x: 100, y: 100 };
  };
  ...
  const openPortal = (ideaId: string, ideaTitle: string) => {
    setPortalState((prev) => {
      // If opening a different idea, reset position to bottom-right
      const newPosition = prev.ideaId === ideaId 
        ? prev.position 
        : typeof window !== 'undefined'
          ? { x: Math.max(20, window.innerWidth - 420), y: Math.max(20, window.innerHeight - 620) }
          : { x: 100, y: 100 };
